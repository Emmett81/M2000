...
#ifdef MITSHM
  if (UseSHM)
    XShmPutImage(Dsp, Wnd, DefaultGC, Img, 0, 0, 0, 0, width, height, False);
  else
#endif
...
{
  int tmp;
  if (newstatus || code == XK_Caps_Lock)
    newstatus = 1;
...
int LoadFont(char *filename)
{
  ...
  for (i = 0; i < 96 * 10; i += 10)
  {
    for (j = 0; j < 10; ++j)
    {
      ...
      {
        d = TempBuf[i + j + 1];
        if ((c & 0x10) && (d & 0x08) && !(d & 0x10))
          l |= 0x0080;
        if ((c & 0x08) && (d & 0x04) && !(d & 0x08))
          l |= 0x0020;
        if ((c & 0x04) && (d & 0x02) && !(d & 0x04))
          l |= 0x0008;
        if ((c & 0x02) && (d & 0x01) && !(d & 0x02))
          l |= 0x0002;
        if ((d & 0x10) && (c & 0x08) && !(d & 0x08))
          l |= 0x0100;
        if ((d & 0x08) && (c & 0x04) && !(d & 0x04))
          l |= 0x0040;
        if ((d & 0x04) && (c & 0x02) && !(d & 0x02))
          l |= 0x0010;
        if ((d & 0x02) && (c & 0x01) && !(d & 0x01))
          l |= 0x0004;
      }
      if (videomode && j != 0)
      {
  ...
  for (i = 96 * 10; i < (96 + 128) * 10; ++i)
  {
    ...
    if (c & 0x01)
      k |= 0x0003;
    FontBuf[i * 4] = FontBuf[i * 4 + 2] = k >> 8;
    FontBuf[i * 4 + 1] = FontBuf[i * 4 + 3] = k;
  }
  free(TempBuf);
  return 1;
}
...
void Keyboard(void)
{
  ...
  if (PausePressed)
  {
    ...
    {
      keyboard_update();
      if (XCheckWindowEvent(Dsp, Wnd, ExposureMask, &E))
        PutImage();
      XPeekEvent(Dsp, &E);
    }
    OldTimer = ReadTimer();
