...
int StartP2000(void)
{
  ...
  if (F)
  {
    if (fread(ROM, 1, 0x1000, F) == 0x1000)
      J = 1;
    fclose(F);
  }
  if (Verbose)
  ...
  if (F)
  {
    if (fread(ROM + 0x1000, 1, 0x4000, F))
      J = 1;
    fclose(F);
  }
  if (Verbose)
  ...
  {
    if (Verbose)
      printf("Opening tape image %s... ", TapeName);
    TapeStream = fopen(TapeName, "a+b");
    if (Verbose)
      puts((TapeStream) ? "OK" : "FAILED");
    if (TapeStream)
      rewind(TapeStream);
  }

  if (Verbose)
    printf("Opening printer stream %s... ",
           (PrnName) ? PrnName : "stdout");
  PrnStream = (PrnName) ? fopen(PrnName, "wb") : stdout;
  if (PrnStream && !PrnName)
    PrnName = "stdout";
  if (Verbose)
    puts((PrnStream) ? "OK" : "FAILED");
...
void OptionsDialogue(void)
{
  ...
  do
  {
    ...
    case 'P':
      ...
      fclose(PrnStream);
      if (Verbose)
        printf("Opening printer log file %s... ", PrnName);
...
#define fileleng 0x6032
  ...
  case 0x04F1:
    ...
    *************************************************************************/
    case 2:
      ...
      if (TapeStream)
      {
        ...
        else if ((j = fgetc(TapeStream)) == EOF)
        {
          rewind(TapeStream);
          Z80_WRMEM(caserror, 0x45);
        }
    ...
    *************************************************************************/
    ...
    default:
      if (Verbose & 4)
        printf("Invalid function number %u\n", R->AF.B.h);
